...
def create_states_from_moves(moves, time, row, line, outputfile):
...
def create_dataset(input_file_with_moves, output_file):
    ...
    with open(input_file_with_moves) as csv_file:
        ...
        for row in csv_reader:
            moves = row[1]
            if (row[0] in (None, "")):  # checks if row is empty
                line += 1
            else:
                line += 1
                b = BughouseEnv()
                create_states_from_moves(moves, row[0], row, line, output_file)
...
create_dataset('bughouse_dataset_small.csv', 'bughouse_testset.csv')
