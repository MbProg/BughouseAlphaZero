...
class BugHouseNet():
    def __init__(self, game, args):
        ...
        # Block 1
        x = VGG_Conv2DBlock(64, (3,3), 'relu', 'same',self.channel_pos, inp, 2, inp_shape=inp_shape)
        # Block 2 
        x = VGG_Conv2DBlock(128, (3,3), 'relu', 'same',self.channel_pos, x, 2)
        # Block 3
        x = VGG_Conv2DBlock(256, (3,3), 'relu', 'same',self.channel_pos, x)
        # Block 4
        x = VGG_Conv2DBlock(512, (3,3), 'relu', 'same',self.channel_pos, x)
        # Block 5 
        x = VGG_Conv2DBlock(512, (3,3), 'relu', 'same',self.channel_pos, x)

        x = Flatten()(x)
        dense_1 = Dense(1028, activation='relu')(x)
